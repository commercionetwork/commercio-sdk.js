import {
  didPowerUpfromWallet
} from "../request_did_power_up_helper";

describe("id/request_did_power_up_helper", () => {
  it("Checks if function 'didPowerUpfromWallet' returns a RequestDidPowerUp", () => {
    const senderDid = "did:com:14zk9u8894eg7fhgw0dsesnqzmlrx85ga9rvnjc";
    const pairwiseDid = "did:com:1ttxcnevaxlk58u4uuec2n7cm44rk9tsgc8tn6m";
    const amount = [{
      amount: "10",
      denom: "ucommercio"
    }];
    const privateKey = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n";
    const governmentRsaPubKey = "-----BEGIN PUBLIC KEY-----\\nbQIDAQAB\n-----END PUBLIC KEY-----\n";

    const expectedValue = {
      claimant: "did:com:14zk9u8894eg7fhgw0dsesnqzmlrx85ga9rvnjc",
      amount: [{
        amount: "10",
        denom: "ucommercio"
      }],
      proof: "<proof>",
      id: "<uuid>",
      proof_key: "<proof_key>"
    };

    const response = didPowerUpfromWallet({
      senderDid: senderDid,
      pairwiseDid: pairwiseDid,
      amount: amount,
      privateKey: privateKey,
      governmentRsaPubKey: governmentRsaPubKey
    });

    expect(response.claimant).toBe(expectedValue.claimant);
    expect(response.amount).toStrictEqual(expectedValue.amount);
    expect(response.id).toHaveLength(36);
  });
});