...
describe("id/id__helper", () => {
  it("Checks if function 'buildRequestDidPowerUpMsgList' returns an array of 'commercio/MsgRequestDidPowerUp'", () => {
    const didPowerUpRequests = [{
    ...
    const response = buildRequestDidPowerUpMsgList(didPowerUpRequests);

    expect(response).toStrictEqual(expectedValue);
  });

  it("Checks if function 'buildSetDidDocumentMsgList' returns an array of 'commercio/MsgSetIdentity'", () => {
    const didDocuments = [{
    ...
    const response = buildSetDidDocumentMsgList(didDocuments);
